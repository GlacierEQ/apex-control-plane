...
def validate_verifier_execution_authority(frontier_authority: Mapping[str, Any], *, resolver: SourceResolver) -> VerifierExecutionResult:
    errors: list[str] = []
    ...
    if not _nonempty(frontier_id):
        return VerifierExecutionResult(False, "VERIFIER_EXECUTION_UNRESOLVED", ("frontier_authority.frontier_id must be non-empty",))

    identity_artifact = frontier_authority.get("verifier_identity_attestation")
    if not isinstance(identity_artifact, Mapping):
        return VerifierExecutionResult(False, "VERIFIER_EXECUTION_UNRESOLVED", ("frontier_authority.verifier_identity_attestation must be an object",))
    identity_bytes, resolution_error = _resolve(resolver, identity_artifact.get("evidence_ref"), prefix="frontier_authority.verifier_identity_attestation")
    if resolution_error:
        return VerifierExecutionResult(False, "VERIFIER_EXECUTION_READBACK_UNRESOLVED", (resolution_error,))
    assert identity_bytes is not None
    identity, parse_error = _json_object(identity_bytes, prefix="frontier_authority.verifier_identity_attestation.evidence")
    if parse_error or identity is None:
        return VerifierExecutionResult(False, "VERIFIER_EXECUTION_UNRESOLVED", (parse_error or "verifier identity unresolved",))
    ...
    if not _nonempty(verifier_ref) or not _nonempty(implementation_sha256):
        errors.append("verifier identity must expose verifier_ref and verifier_implementation_sha256")

    artifact = frontier_authority.get("verifier_execution_attestation")
    if not isinstance(artifact, Mapping):
        errors.append(f"{prefix} must contain independently resolved provider execution evidence")
    else:
        evidence_bytes, resolution_error = _resolve(resolver, artifact.get("evidence_ref"), prefix=prefix)
        if resolution_error:
            ...
            evidence, parse_error = _json_object(evidence_bytes, prefix=f"{prefix}.evidence")
            if parse_error:
                errors.append(parse_error)
            elif evidence is not None:
                for key, expected in (("frontier_id", frontier_id), ("verifier_ref", verifier_ref), ("verifier_implementation_sha256", implementation_sha256), ("verdict", "verifier_executed")):
                    if evidence.get(key) != expected:
                        errors.append(f"{prefix}.evidence.{key} must equal {expected!r}")
                provider = evidence.get("provider")
                run_ref = evidence.get("run_ref")
                if not _nonempty(provider) or not _nonempty(run_ref):
                    errors.append(f"{prefix}.evidence.provider and run_ref must be non-empty")
                else:
    ...
    if errors:
        ...
        return VerifierExecutionResult(False, status, tuple(dict.fromkeys(errors)))
