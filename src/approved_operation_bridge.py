...
class ProviderExecutionObservation:
    ...
    def __post_init__(self) -> None:
        if not self.source_refs:
            raise ApprovedOperationError("provider execution observation requires a source reference")
        if self.observed_at.tzinfo is None or self.observed_at.utcoffset() is None:
            raise ApprovedOperationError("provider execution observation time must include a timezone")
...
def _mapping(value: Any, name: str, *, required_values: bool = True) -> Mapping[str, Any]:
...
def _execution_evidence(value: Any) -> ExecutionEvidence:
    ...
    try:
        return ExecutionEvidence(
            ...
            epistemic_state=EpistemicState(_text(raw.get("epistemic_state"), "execution_evidence.epistemic_state")),
            ...
            recovery_checkpoint_verified=raw.get("recovery_checkpoint_verified") is True,
        ...
        raise ApprovedOperationError("execution_evidence contains an unsupported safety value") from exc
...
def validate_approved_action_request(
    ...
    *,
    ...
    if write_rule.get("idempotency_required") is not True:
        raise ApprovedOperationError("catalogued provider operation lacks idempotency enforcement")
    if write_rule.get("terminal_readback_required") is not True:
        raise ApprovedOperationError("catalogued provider operation lacks terminal readback enforcement")
    ...
    supplied_scope = _text(approval.get("approval_scope_sha256"), "approval.approval_scope_sha256")
    if not _is_sha256(supplied_scope):
        raise ApprovedOperationError("approval.approval_scope_sha256 must be a lowercase SHA-256 digest")
    if supplied_scope != expected_scope:
        raise ApprovedOperationError("approval scope does not match the immutable action request")

    current = now.astimezone(UTC) if now is not None else None
    if current is not None:
        age_seconds = (current - base.approved_at).total_seconds()
        if age_seconds < -300:
            raise ApprovedOperationError("approval.approved_at is materially in the future")
        if age_seconds > catalog.maximum_receipt_age_seconds:
            raise ApprovedOperationError("approval is stale for the current connector policy")

    readiness = evaluate_execution(execution_evidence)
    if not readiness.allowed:
        reasons = "; ".join(readiness.reasons) or readiness.decision.value
        raise ApprovedOperationError(f"provider operation is not ready for execution: {reasons}")
    if execution_evidence.operation != f"{base.connector}.{base.operation}":
        raise ApprovedOperationError("execution_evidence.operation must match the requested provider operation")
...
def build_execution_receipt(
    *,
    ...
    if state == "success" and readback is None:
        raise ApprovedOperationError("successful provider execution requires terminal readback")
    if state == "success" and not verification_passed:
        raise ApprovedOperationError("successful provider execution requires verified terminal readback")
...
def validate_execution_receipt(
    ...
    if payload.get("external_action_authorized") is not True:
        raise ApprovedOperationError("execution receipt must record an exact approved action")
    for field, expected in (
        ...
        if _text(payload.get(field), f"execution receipt {field}") != expected:
            raise ApprovedOperationError(f"execution receipt {field} does not match the approved action")

    state = _text(payload.get("result_state"), "execution receipt result_state")
    if state not in {"success", "failure"}:
        raise ApprovedOperationError("execution receipt result_state must be success or failure")
    executed_at = _parse_time(payload.get("executed_at"), "execution receipt executed_at")
    result_target = _mapping(payload.get("result_target"), "execution receipt result_target")
    execution_refs = _refs(payload.get("execution_source_refs"), "execution receipt execution_source_refs")
    execution_digest = payload.get("execution_content_sha256")
    if execution_digest is not None and (not isinstance(execution_digest, str) or not _is_sha256(execution_digest)):
        raise ApprovedOperationError("execution receipt execution_content_sha256 is invalid")
    ...
    readback_at = None if readback_at_raw is None else _parse_time(readback_at_raw, "execution receipt readback_at")
    ...
    return ConnectorExecutionReceipt(
        execution_receipt_id=_text(payload.get("execution_receipt_id"), "execution_receipt_id"),
        action_request_id=action.action_request_id,
        connector=action.connector,
        operation=action.operation,
        idempotency_key=action.idempotency_key,
