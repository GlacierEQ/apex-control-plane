...
`plan_ref` is a source-authored plan-identity pointer required for `plan_batch`; it is not a second approval service. The independently resolved Operator source record controls the envelope. Constituent membership is enforced against its connector, operation/action-class, target, provider-input, and consequence constraints, while the concrete action digest separately binds evidence references and idempotency for execution/readback integrity.
