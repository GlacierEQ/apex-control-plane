...
const TOOLS: ToolDef[] = [
  {
    name: "memory_federation_health",
    description: "Read the live GlacierEQ memory-federation backend state and adapter configuration without exposing secrets.",
    action: "health",
    write: false,
    inputSchema: { type: "object", properties: {}, additionalProperties: false },
  },
  {
    name: "memory_federation_preview",
    description: "Preview one durable memory-federation sync event, including provenance-bound memory/backend policy and whether the direct adapter is configured. Performs no provider call.",
    action: "preview",
    write: false,
    inputSchema: { type: "object", properties: { event_id: { type: "integer", minimum: 1 } }, required: ["event_id"], additionalProperties: false },
  },
  {
    name: "memory_federation_dispatch",
    description: "Execute one claimable memory-federation event through the existing direct provider adapter. Preserves namespace/sensitivity policy, duplicate suppression, leases, provider receipts, error classification, and durable bindings.",
    action: "dispatch",
    write: true,
    inputSchema: { type: "object", properties: { event_id: { type: "integer", minimum: 1 } }, required: ["event_id"], additionalProperties: false },
  },
...
async function invokeDispatcher(tool: ToolDef, args: Json): Promise<Json> {
  ...
  const res = await fetch(DISPATCHER_URL, {
    ...
    headers: {
      ...
      "x-glaciereq-origin": "apex-direct-memory-mcp",
    },
  ...
  if (!res.ok) throw new Error(`memory_dispatcher_http_${res.status}:${typeof data === "string" ? data.slice(0, 1000) : JSON.stringify(data).slice(0, 1000)}`);
...
Deno.serve(async (req: Request) => {
  ...
  if (method === "initialize") return response(ok(id, {
    protocolVersion: PROTOCOL_VERSION,
    capabilities: { tools: { listChanged: false } },
    serverInfo: { name: "glaciereq-direct-memory", version: "1.0.0" },
    instructions: "GlacierEQ-owned direct Memory MCP over the durable memory-federation dispatcher. No Smithery dependency. Provider-specific policy, leases, duplicate suppression, receipts, bindings, and error classification remain authoritative in the dispatcher.",
  }));
  ...
  if (method === "tools/call") {
    ...
    try {
      const value = await invokeDispatcher(tool, args);
      return response(ok(id, { content: [{ type: "text", text: JSON.stringify(value) }], structuredContent: value, isError: false }));
    } catch (e) {
      const message = e instanceof Error ? e.message : String(e);
      return response(ok(id, { content: [{ type: "text", text: message }], structuredContent: { ok: false, error: message, tool: name }, isError: true }));
    }
  }
