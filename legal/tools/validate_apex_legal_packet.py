...
def validate(packet: dict[str, Any]) -> list[tuple[str, str, str, str]]:
    ...
    for key in ("matter_id", "generated_at"):
        if not isinstance(packet.get(key), str) or not packet[key].strip():
            out.append(
                finding("ERROR", "MISSING_REQUIRED", key, "must be a non-empty string")
            )
    ...
    for collection in COLLECTIONS:
        ...
        if not isinstance(items, list):
            out.append(
                finding("ERROR", "INVALID_COLLECTION", collection, "must be an array")
            )
            continue
        ...
        for pos, item in enumerate(items):
            ...
            if not isinstance(item, dict):
                out.append(
                    finding("ERROR", "INVALID_OBJECT", path, "must be an object")
                )
                continue
            item_id = item.get("id")
            if not isinstance(item_id, str) or not item_id.strip():
                out.append(
                    finding("ERROR", "MISSING_ID", path, "non-empty id required")
                )
                continue
            if item_id in index:
    ...
    def refs(path: str, values: Any, target: str) -> None:
        ...
        if not isinstance(values, list):
            out.append(
                finding("ERROR", "INVALID_REF_LIST", path, "must be an array of ids")
            )
            return
    ...
    for aid, allegation in indexes.get("allegations", {}).items():
        ...
        if (
            not isinstance(allegation.get("label"), str)
            or not allegation["label"].strip()
        ):
            out.append(
                finding(
                    "ERROR",
        ...
        if not isinstance(elements, list) or not elements:
            out.append(
                finding(
                    ...
                    "MISSING_ELEMENTS",
                    f"{base}.elements",
                    "at least one legal element required",
                )
            )
        else:
                ...
                if (
                    ...
                    out.append(
                        finding(
                            ...
                            "INVALID_ELEMENT",
                            ep,
                            "named element object required",
                        )
                    )
                    continue
                refs(
                    f"{ep}.supporting_evidence_ids",
                    element.get("supporting_evidence_ids", []),
                    "evidence",
                )
                refs(
                    f"{ep}.contrary_evidence_ids",
                    element.get("contrary_evidence_ids", []),
                    "evidence",
                )
                refs(
                    f"{ep}.proof_gap_ids",
                    element.get("proof_gap_ids", []),
                    "proof_gaps",
                )
        refs(f"{base}.evidence_ids", allegation.get("evidence_ids", []), "evidence")
        refs(f"{base}.actor_ids", allegation.get("actor_ids", []), "actors")
        refs(
            f"{base}.authority_ids", allegation.get("authority_ids", []), "authorities"
        )
    ...
    for eid, event in indexes.get("chronology", {}).items():
        ...
        refs(
            f"{base}.source_evidence_ids",
            event.get("source_evidence_ids", []),
            "evidence",
        )
        if event.get("time_basis") == "EXACT" and not event.get("timestamp"):
            out.append(
                finding(
                    "ERROR",
                    "EXACT_TIME_NO_TIMESTAMP",
                    f"{base}.timestamp",
                    "EXACT requires timestamp",
                )
            )
        ...
        for field in ("left_source_id", "right_source_id"):
            ...
            if not isinstance(ref, str) or ref not in evidence:
                out.append(
                    finding(
                        "ERROR",
                        "BROKEN_CONTRADICTION_SOURCE",
                        f"contradictions[{cid}].{field}",
                        repr(ref),
                    )
                )
    ...
    for authid, authority in indexes.get("authorities", {}).items():
        ...
        if authority.get("temporal_fit") in (None, "UNKNOWN"):
            out.append(
                finding(
                    "WARN",
                    "UNKNOWN_TEMPORAL_FIT",
                    f"{base}.temporal_fit",
                    "verify law/rule version governing event",
                )
            )
    ...
    for actorid, actor in indexes.get("actors", {}).items():
        ...
        if not actor.get("responsibility_modes"):
            out.append(
                finding(
                    "WARN",
                    "UNMAPPED_RESPONSIBILITY",
                    f"{base}.responsibility_modes",
                    "map responsibility chain",
                )
            )
    ...
    for gid, gap in indexes.get("proof_gaps", {}).items():
        if not gap.get("target"):
            out.append(
                finding(
                    "WARN",
                    "NO_ACQUISITION_TARGET",
                    f"proof_gaps[{gid}].target",
                    "map to retrieval/discovery/preservation/authority check",
                )
            )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Validate an APEX legal analysis packet"
    )
    ...
    if not isinstance(packet, dict):
        print(
            "ERROR PACKET_LOAD top-level packet must be a JSON object", file=sys.stderr
        )
        return 2
