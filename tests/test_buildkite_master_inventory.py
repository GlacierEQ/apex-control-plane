...
p = ROOT / "scripts/buildkite_master_inventory.py"
s = importlib.util.spec_from_file_location("bmi", p)
...
def test_redact_removes_sensitive_keys():
    ...
    assert (
        "token" not in x
        and "secret" not in x["nested"]
        and x["nested"]["key"] == "SAFE"
    )
...
def test_scope_state_preserves_unknown():
    assert (
        m.scope_state({"read_builds"}, "read_agents")["status"]
        == "UNVERIFIED_MISSING_SCOPE"
    )
