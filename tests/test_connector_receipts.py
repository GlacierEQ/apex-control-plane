...
def test_catalog_loads_and_prohibits_credential_storage():
    ...
    assert catalog.connectors["dropbox"]["read_operations"] == (
        "file.search", "file.list", "file.metadata.read", "file.extract_text", "file.download_preserve",
    )
...
def test_read_receipt_rejects_unknown_operation_stale_time_and_action_claim():
    ...
    with pytest.raises(ConnectorReceiptError, match="stale"):
        validate_read_receipt(read_receipt(observed_at=(NOW - timedelta(days=2)).isoformat()), catalog, now=NOW)
    with pytest.raises(ConnectorReceiptError, match="non-authorizing"):
        validate_read_receipt(read_receipt(external_action_authorized=True), catalog, now=NOW)


def test_build_read_request_only_allows_catalogued_profile_operation():
    catalog = load_connector_catalog(CATALOG_PATH)
    request = build_read_request(
        connector="github", operation="branch_protection.read", profile="repository_integrity",
        target={"repository": "GlacierEQ/apex-control-plane"}, catalog=catalog, requested_at=NOW,
    )
    assert request["external_action_authorized"] is False
    assert request["requested_at"] == "2026-08-22T12:00:00Z"
    with pytest.raises(ConnectorReceiptError, match="not allowed"):
        build_read_request(
            connector="github", operation="issue.create", profile="repository_integrity",
            target={"repository": "GlacierEQ/apex-control-plane"}, catalog=catalog, requested_at=NOW,
        )


def test_inactive_write_route_remains_blocked_even_with_full_authorization_record():
    catalog = load_connector_catalog(CATALOG_PATH)
    with pytest.raises(ConnectorReceiptError, match="inactive"):
        validate_action_request(
            action_request(connector="notion", operation="page.create", target={"parent_page_id": "page-001"}), catalog,
        )


def test_enabled_write_route_requires_attributable_authorization_and_stated_consequence(tmp_path):
    ...
    with pytest.raises(ConnectorReceiptError, match="approval_reference"):
        validate_action_request(
            action_request(approval={"approved_by": "GlacierEQ", "approved_at": NOW.isoformat()}), catalog,
        )
...
def test_runtime_admits_read_receipt_with_safe_audit_details_and_deduplicates():
    ...
    runtime = CaseBrainOrchestrator(
        producer=Producer(repo="GlacierEQ/apex-control-plane", commit_sha="a" * 40, component="connector-receipt-test")
    )
    ...
    assert duplicate == {"status": "duplicate", "receipt_id": "receipt-dropbox-001", "external_action_authorized": False}
...
def test_action_proposal_remains_non_authorizing_when_route_is_active():
    ...
    proposal = build_action_proposal(
        connector="github", operation="issue.create",
        target={"repository": "GlacierEQ/apex-control-plane"},
        consequence="Creates one issue visible to repository collaborators.",
        evidence_refs=["receipt-github-001"], catalog=catalog,
    )
