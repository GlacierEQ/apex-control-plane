...
def main() -> int:
    ...
    minimum_contract_version = str(source["minimum_contract_version"])
    require(contract_id == source["contract_id"], "contract id mismatch between manifest and source")
    require(
        version_tuple(contract_version) >= version_tuple(minimum_contract_version),
        f"contract version {contract_version} is below portability minimum {minimum_contract_version}",
    )
    require(
        version_tuple(minimum_contract_version) >= version_tuple("1.3.0"),
        "portability manifest permits a pre-hardening Jack contract",
    )
    ...
    require(projection["adapter_may_expand_operator_operation_scope"] is False, "adapter objective-scope rewrite enabled")
    ...
    require(
        projection["adapter_may_execute_mission_aligned_hardening_when_source_contract_authorizes"] is True,
        "portable adapter cannot execute source-authorized mission-aligned hardening",
    )
    ...
    require(
        asset["inspection_may_expand_into_mission_aligned_hardening_without_reconfirmation"] is True,
        "mission-aligned inspection hardening is not portable",
    )
    ...
    require("preserve_full_supportable_allegation_field: true" in contract_text, "full allegation field is not portable")
    require("no_representative_allegation_pruning: true" in contract_text, "allegation-pruning prohibition is not portable")
    require("filing_subset_does_not_shrink_case_graph: true" in contract_text, "case-graph preservation is not portable")
