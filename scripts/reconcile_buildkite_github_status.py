...
def reconcile(payload: dict[str, Any], expected_context: str) -> Projection:
    ...
    matches = [
        s
        for s in statuses
        if isinstance(s, dict) and s.get("context") == expected_context
    ]
    if not matches:
    ...
    if state != "success":
        return Projection(
            "NON_SUCCESS", expected_context, state, target, None, None, None
        )
    m = BUILDKITE_URL_RE.match(str(target or ""))
    if not m:
        return Projection(
            "INVALID_TARGET", expected_context, state, target, None, None, None
        )
    org, pipeline, number = m.groups()
    return Projection(
        "VERIFIED_SUCCESS_PROJECTION",
        expected_context,
        state,
        target,
        org,
...
def fetch_status(repo: str, sha: str, token: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise RuntimeError(
            f"GitHub status read failed HTTP {exc.code}: {exc.read().decode(errors='replace')[:500]}"
        ) from exc
...
def main() -> int:
    ...
    if a.input:
        ...
        if not token:
            print("GITHUB_TOKEN required when --input is omitted")
            return 2
        payload = fetch_status(a.repo, a.sha, token)
    result = {
        "schema": "glaciereq.apex.buildkite-github-projection.v1",
        "repository": a.repo,
        "commit": a.sha,
        **asdict(reconcile(payload, a.context)),
    }
    if a.output:
        a.output.parent.mkdir(parents=True, exist_ok=True)
        a.output.write_text(json.dumps(result, indent=2, sort_keys=True) + "\n")
    print(json.dumps(result, indent=2, sort_keys=True))
    return 0 if result["status"] == "VERIFIED_SUCCESS_PROJECTION" else 1
