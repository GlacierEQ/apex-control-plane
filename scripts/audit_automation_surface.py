#!/usr/bin/env python3
"""Audit active GitHub Actions for unattended source mutation hazards.

Provider secrets are not inherently unsafe in scheduled read-only workflows.
This audit targets workflows that can rewrite repository source/history without
an explicit manual execution path, plus destructive Git operations anywhere.
"""
...
def _workflow_files(root: Path) -> list[Path]:
    if not root.is_dir():
        return []
    return sorted(path for path in root.iterdir() if path.is_file() and path.suffix.casefold() in {".yml", ".yaml"})
...
def audit_workflow(path: Path) -> list[Finding]:
    ...
    if scheduled and contents_write:
        findings.append(Finding("ERROR", "SCHEDULED_CONTENTS_WRITE", name, "scheduled workflow has repository contents: write permission"))
    if scheduled and persists_credentials:
        findings.append(Finding("ERROR", "SCHEDULED_PERSISTED_GIT_CREDENTIALS", name, "scheduled workflow persists checkout credentials"))
    if scheduled and git_push:
        findings.append(Finding("ERROR", "SCHEDULED_GIT_PUSH", name, "scheduled workflow executes git push"))
    if push_triggered and contents_write and git_push:
        findings.append(Finding("ERROR", "PUSH_TRIGGERED_SELF_WRITE", name, "push-triggered workflow can push repository contents back to the repository"))

    for code, pattern in DESTRUCTIVE_PATTERNS:
        if pattern.search(text):
            findings.append(Finding("ERROR", f"DESTRUCTIVE_GIT_{code.upper()}", name, "active workflow contains a destructive repository-history/worktree command"))

    if contents_write and not manual and not scheduled and not push_triggered:
        findings.append(Finding("WARN", "WRITE_PERMISSION_WITHOUT_EXPLICIT_MANUAL_TRIGGER", name, "workflow can write repository contents without workflow_dispatch; inspect event scope"))
    return findings
...
def audit(root: Path = WORKFLOW_ROOT) -> dict[str, object]:
    ...
    return {
        "schema": "glaciereq.apex.automation-safety.v2",
        ...
        "policy": {
            ...
            "scheduled_persisted_git_credentials": "forbidden",
            "scheduled_provider_secrets": "permitted_when_repository_source_remains_read_only",
