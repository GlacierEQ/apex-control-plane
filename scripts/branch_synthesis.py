...
PATH_SIGNALS = (
    ...
    (
        re.compile(
            r"(^|/)(evidence|forensic|timeline|contradiction|docket)(/|$)", re.I
        ),
        7,
        "legal-intelligence",
    ),
    (
        re.compile(r"\.(py|ts|tsx|js|jsx|rs|go|java|kt|swift|rb|cs|cpp|c|h)$", re.I),
        4,
        "executable-code",
    ),
...
class GitHubAPI:
    ...
    def _request(self, url: str) -> urllib.request.Request:
        headers = {
            ...
            "User-Agent": "GlacierEQ-Branch-Synthesis/1",
        }
        if self.token:
    ...
    def _read(self, url: str) -> tuple[Any, Mapping[str, str]]:
        try:
            ...
            raise BranchSynthesisError(
                f"GitHub GET failed with HTTP {exc.code}: {body[:240]}"
            ) from exc
        except (
    ...
    def _identity(repository: str) -> tuple[str, str]:
        if not REPO_RE.fullmatch(repository):
            raise BranchSynthesisError(f"invalid repository identity: {repository!r}")
    ...
    def repository(self, repository: str) -> Mapping[str, Any]:
        ...
        if not isinstance(payload, Mapping):
            raise BranchSynthesisError(f"{repository} metadata was not an object")
        return payload

    def branches(self, repository: str) -> list[Mapping[str, Any]]:
        url: str | None = (
            f"{self.api_url}/{self._path(repository, '/branches')}?per_page=100"
        )
        rows = []
        while url:
            ...
            if not isinstance(payload, list):
                raise BranchSynthesisError(
                    f"{repository} branch listing was not an array"
                )
            rows.extend(row for row in payload if isinstance(row, Mapping))
            if len(rows) > MAX_BRANCHES:
                raise BranchSynthesisError(
                    f"{repository} exceeds {MAX_BRANCHES} branches"
                )
    ...
    def compare(self, repository: str, base: str, head: str) -> Mapping[str, Any]:
        ...
        if not isinstance(payload, Mapping):
            raise BranchSynthesisError(
                f"{repository}:{head} comparison was not an object"
            )
        return payload

    def tree(self, repository: str, commit_sha: str) -> tuple[Mapping[str, str], bool]:
        commit, _ = self._read(
            f"{self.api_url}/{self._path(repository, f'/git/commits/{commit_sha}')}"
        )
        if not isinstance(commit, Mapping) or not isinstance(
            commit.get("tree"), Mapping
        ):
            raise BranchSynthesisError(
                f"{repository}:{commit_sha} did not expose a Git tree"
            )
        tree_sha = _sha(commit["tree"].get("sha"), f"{repository} tree")
        tree, _ = self._read(
            f"{self.api_url}/{self._path(repository, f'/git/trees/{tree_sha}')}?recursive=1"
        )
        if not isinstance(tree, Mapping) or not isinstance(tree.get("tree"), list):
            raise BranchSynthesisError(
                f"{repository}:{tree_sha} tree response was malformed"
            )
        entries = tree["tree"]
        if len(entries) > MAX_TREE_ENTRIES:
            raise BranchSynthesisError(
                f"{repository} default tree exceeds {MAX_TREE_ENTRIES} entries"
            )
        blobs = {
            ...
            and row.get("path")
            and SHA_RE.fullmatch(str(row.get("sha") or ""))
        }
...
class RepositoryInventory:
    ...
    preservation_set: tuple[str, ...]
    retirement_candidates: tuple[str, ...]
    branch_families: Mapping[str, tuple[str, ...]]
    branches: tuple[BranchInventory, ...]
...
def _sha(value: Any, label: str) -> str:
    ...
    if not SHA_RE.fullmatch(sha):
        raise BranchSynthesisError(f"invalid {label} SHA: {value!r}")
    return sha
...
def _count(value: Any, label: str) -> int:
    try:
        ...
        raise BranchSynthesisError(f"invalid {label}: {value!r}") from exc
    if parsed < 0:
        raise BranchSynthesisError(f"invalid {label}: {parsed}")
    return parsed
...
def branch_family_key(name: str) -> str:
    ...
    parts = name.split("/")
    original = parts[-1].strip().casefold()
    leaf = original
    while True:
        before = leaf
        for pattern in (DATE_END, HEX_END, VERSION_END):
            leaf = pattern.sub("", leaf)
        tokens = [token for token in re.split(r"[-_]+", leaf) if token]
        while tokens and tokens[-1] in ITERATION_END:
            tokens.pop()
        leaf = "-".join(tokens)
        if leaf == before:
            break
    stem = leaf or original
    prefix = "/".join(part.casefold() for part in parts[:-1])
...
def _paths(payload: Mapping[str, Any]) -> tuple[tuple[str, ...], bool]:
    ...
    if not isinstance(files, list):
        raise BranchSynthesisError("comparison files must be an array")
    paths = {
...
def _content_relation(
    payload: Mapping[str, Any], default_tree: Mapping[str, str], tree_complete: bool
) -> tuple[str, int, int]:
    files = payload.get("files")
...
def _relation(
    ...
    if status == "identical":
        return (
            "IDENTICAL",
            "RETIRE_AFTER_REACHABILITY_PROOF",
            ("head equals current default",),
            0,
        )
    if status == "behind" and ahead == 0:
        return (
            "ABSORBED",
            "RETIRE_AFTER_REACHABILITY_PROOF",
            ("no unique commits; head is reachable from current default",),
            0,
        )
    if status == "ahead" and ahead > 0 and behind == 0:
        return (
            "FORWARD_DONOR",
            "PRESERVE_AND_SYNTHESIZE",
            (f"{ahead} unique commit(s) beyond current default",),
            30,
        )
    if status == "diverged" and ahead > 0 and behind > 0:
        return (
            "DIVERGED_DONOR",
            "PRESERVE_AND_FRESH_SYNTHESIZE",
            (
                f"{ahead} unique commit(s)",
                f"missing {behind} newer default commit(s); wholesale merge is unsafe",
            ),
            40,
        )
    return (
        ...
        (f"ambiguous compare state {status!r}: ahead={ahead}, behind={behind}",),
        20,
    )
...
def _branch(
    ...
    default: str,
    default_sha: str,
    default_tree: Mapping[str, str],
    tree_complete: bool,
    raw: Mapping[str, Any],
) -> BranchInventory:
    name = str(raw.get("name") or "").strip()
    commit = raw.get("commit")
    if not name or not isinstance(commit, Mapping):
        raise BranchSynthesisError(f"{repository} returned malformed branch metadata")
    head = _sha(commit.get("sha"), f"{repository}:{name} head")
    protected = bool(raw.get("protected"))
    family = branch_family_key(name)
    if name == default:
        if head != default_sha:
            raise BranchSynthesisError(
                f"{repository}:{default} default-head sources disagree"
            )
        return BranchInventory(
            ...
            (),
            0,
            ("current default head is the synthesis floor",),
        )
    payload = reader.compare(repository, default, name)
    ahead = _count(payload.get("ahead_by"), "ahead_by")
    behind = _count(payload.get("behind_by"), "behind_by")
    relation, action, reasons, base_score = _relation(
        str(payload.get("status") or "UNKNOWN"), ahead, behind
    )
    paths, truncated = _paths(payload)
    content_relation, verified, delta = _content_relation(
        payload, default_tree, tree_complete and not truncated
    )
    if (
        relation in {"FORWARD_DONOR", "DIVERGED_DONOR"}
        and content_relation == "CONTENT_EQUIVALENT"
    ):
        action = "RETIRE_AFTER_CONTENT_EQUIVALENCE_PROOF"
        reasons = reasons + (
            "every compared branch file effect already matches current default",
        )
        base_score = 0
    ...
    return BranchInventory(
        ...
        delta,
        signals,
        round(base_score + min(ahead, 20) + path_score + (2 if protected else 0), 2),
        reasons,
    )


def _digest(
    repository: str, default: str, default_sha: str, rows: Iterable[BranchInventory]
) -> str:
    payload = {
        "repository": repository,
        "default_branch": default,
        "default_head_sha": default_sha,
...
def inventory_repository(reader: GitHubReader, repository: str) -> RepositoryInventory:
    _valid_repo(repository)
    repo = reader.repository(repository)
    default = str(repo.get("default_branch") or "").strip()
    if not default:
        raise BranchSynthesisError(f"{repository} did not expose a default branch")
    raw = reader.branches(repository)
    if len(raw) > MAX_BRANCHES:
        raise BranchSynthesisError(f"{repository} exceeds {MAX_BRANCHES} branches")
    names = [str(row.get("name") or "").strip() for row in raw]
    if len(names) != len(set(names)):
        raise BranchSynthesisError(
            f"{repository} branch listing contains duplicate names"
        )
    defaults = [row for row in raw if str(row.get("name") or "").strip() == default]
    if len(defaults) != 1 or not isinstance(defaults[0].get("commit"), Mapping):
        raise BranchSynthesisError(
            f"{repository} expected one valid default branch row"
        )
    ...
    if (
        ...
        and _sha(embedded.get("sha"), f"{repository} default") != default_sha
    ):
        raise BranchSynthesisError(f"{repository} default-head sources disagree")
    ...
    preserve = tuple(
        sorted(
            (row.name for row in rows if row.action in preserve_actions),
            key=str.casefold,
        )
    )
    retire = tuple(
        sorted(
            (
                ...
                in {
                    "RETIRE_AFTER_REACHABILITY_PROOF",
                    "RETIRE_AFTER_CONTENT_EQUIVALENCE_PROOF",
                }
            ),
            key=str.casefold,
        )
    ...
    families_out = {
        key: tuple(sorted(members, key=str.casefold))
    ...
    return RepositoryInventory(
        ...
        default_sha,
        len(rows),
        sum(row.relation in {"FORWARD_DONOR", "DIVERGED_DONOR"} for row in rows),
        sum(
            row.relation in {"IDENTICAL", "ABSORBED"} and row.name != default
            for row in rows
        ),
        sum(row.relation == "REVIEW_REQUIRED" for row in rows),
        preserve,
        retire,
        families_out,
        rows,
        _digest(repository, default, default_sha, rows),
    )
...
def synthesis_report(inventories: Iterable[RepositoryInventory]) -> dict[str, Any]:
    ...
    return {
        "schema": "APEX_BRANCH_SYNTHESIS_V1",
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "semantics": {
            "baseline": "current default is the floor",
            "preservation": "unique/diverged/ambiguous branches remain donors until composed",
            "retirement": "reachable heads or fully content-equivalent replayed heads qualify; this tool never mutates refs",
            "privacy": "changed paths are used in-memory for scoring but suppressed from reports; no source bytes or legal narratives",
        },
        "repository_count": len(rows),
        "branch_count": sum(row.branch_count for row in rows),
        "donor_count": sum(row.donor_count for row in rows),
        "absorbed_count": sum(row.absorbed_count for row in rows),
        "review_required_count": sum(row.review_required_count for row in rows),
        "repositories": [_report_repository(row) for row in rows],
    }
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Build a capability-preserving branch synthesis manifest"
    )
    parser.add_argument(
        "--repo",
        action="append",
        required=True,
        help="owner/name; repeat for multiple repositories",
    )
    parser.add_argument("--output", required=True)
    parser.add_argument("--token-env", default="APEX_GITHUB_TOKEN")
    parser.add_argument("--api-url", default="https://api.github.com")
    args = parser.parse_args(argv)
    client = GitHubAPI(
        os.environ.get(args.token_env) or os.environ.get("GITHUB_TOKEN"),
    ...
    try:
        write_report(
            args.output,
            synthesis_report(
                inventory_repository(client, repo) for repo in dict.fromkeys(args.repo)
            ),
        )
    except (BranchSynthesisError, OSError) as exc:
